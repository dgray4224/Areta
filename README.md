...
- **Rebrand loose ends** (see "Areta rebrand" status above): the GitHub repo name still says `lifeos`; `getlifeos.tech` is still live with no redirect to `areta-ai.com` and hasn't been canceled; `CLAUDE.md` intentionally still says LifeOS throughout.
