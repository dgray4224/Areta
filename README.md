...
- The "Confirm signup" email template (Authentication → Emails → Templates) is a custom branded template — editable now that custom SMTP is on (Supabase locks template editing behind custom SMTP on the free/default mailer).
- `signUpWithPassword` (`platform/auth/actions.ts`) passes `emailRedirectTo` pointing at `/auth/confirm`, and that route (`app/auth/confirm/route.ts`) handles both the PKCE `code` and `token_hash` confirmation formats Supabase might send — the same route also handles the password-reset link's redirect (`requestPasswordReset` points it at `/auth/confirm?next=/reset-password`), so it works regardless of which flow triggered it.
