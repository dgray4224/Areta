...
import { createMealProgram } from "@/domains/trainermealprogram/service";
import { FormField, TextInput, TextArea } from "@/platform/ui/FormField";
import { Button } from "@/platform/ui/Button";

export function NewMealProgramForm() {
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const result = await createMealProgram({ name, description });
      if (!result.ok) {
        setError(result.error);
        return;
      }
      router.push(`/trainer/meal-programs/${result.data.id}`);
    });
  };
