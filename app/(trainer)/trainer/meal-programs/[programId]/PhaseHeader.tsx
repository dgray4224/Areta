...
import { updatePhase } from "@/domains/trainermealprogram/service";
import { FormField, TextInput, TextArea } from "@/platform/ui/FormField";
import { Button } from "@/platform/ui/Button";
import { DeletePhaseButton } from "./DeletePhaseButton";
import type { TrainerMealProgramPhase } from "@/domains/trainermealprogram/types";

/** View + in-place edit for a phase's own name/focus/length/final flag --
 * mirrors the workout side's PhaseHeader.tsx exactly, including the
 * "row replaces itself with the edit form" pattern. */
export function PhaseHeader({ phase, phaseNumber }: { phase: TrainerMealProgramPhase; phaseNumber: number }) {
    ...
    <div className="mb-3 flex items-start justify-between">
      <div>
        ...
        <p className="text-xs text-neutral-400">
          ...
          {phase.isFinal
            ? "nothing's scheduled — assign a new program to continue past the end date"
