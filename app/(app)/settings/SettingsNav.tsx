 ...
 * admin portal (desktop also gets one in AppHeader); trainers/reviewers
 * reach it this way rather than needing to know the URL. */
      ...
      {tabs.map((tab) => {
        ...
        return (
          <Link
            key={tab.href}
            href={tab.href}
            className={`whitespace-nowrap border-b-2 px-3 py-2 text-sm ${navTabClass(active)}`}
          >
            {tab.label}
          </Link>
        );
