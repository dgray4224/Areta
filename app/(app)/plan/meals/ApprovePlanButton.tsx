...
export function ApprovePlanButton({ userId }: { userId: string }) {
  ...
  const onClick = () => {
    ...
    startTransition(async () => {
      const result = await approveMealPlanAndGenerateDownstream(userId);
      if (!result.ok) {
  ...
  return (
    <div className="space-y-2">
      ...
      >
        {isPending ? "Approving…" : "Approve plan and build grocery + prep list"}
      </button>
